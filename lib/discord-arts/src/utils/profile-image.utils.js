...
async function genTextAndAvatar(data, options, avatarData) {
  ...
  // Move up the username and custom tag if there is rankData
  if (options?.rankData) {
    ...
    ctx.fillStyle = options?.usernameColor
      ...
      : "#FFFFFF";
    if (tag !== null) {
      ctx.fillText(username, 300, 100); // Adjusted y coordinate for username
    } else {
    ...
    if (tag !== null) {
      ...
      ctx.fillStyle = options?.tagColor
        ? parseHex(options.tagColor)
        : "#dadada";
      ctx.fillText(tag, 300, 160); // Adjusted y coordinate for custom tag
    }

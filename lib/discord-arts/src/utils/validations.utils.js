function parseImg(imgString) {
  if (!imgString || typeof imgString !== "string") {
    throw new Error(
      `Discord Arts | Invalid custom badge ('${imgString || undefined}') must be an image file 'png | jpg | gif'`,
    );
  }
  ...
  if (!imgCheck)
    throw new Error(
      `Discord Arts | Invalid customBackground ('${imgString || undefined}') must be an image file 'png | jpg | gif'`,
    );
...
function parsePng(imgString) {
  if (!imgString || typeof imgString !== "string") {
    throw new Error(
      `Discord Arts | Invalid custom badge ('${imgString || undefined}') must be a png file`,
    );
  }
  const URL = imgString.split(".");
  const imgType = URL[URL.length - 1];
